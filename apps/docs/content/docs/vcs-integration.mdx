...
1. Create an access token in your provider with read access to the repositories you want mirrored:
   ...
   - **GitLab**: a personal (or group/project) access token with the `api` scope. Review replies and discussion resolution require write API access.
...
- **Triggers**: enable **Merge request events**, **Comments** (Note events), and **Pipeline events** to mirror CI.
...
- **GitLab review discussions** — a new human MR discussion resumes the linked issue's assigned agent once. After pushing a fix, the agent can reply to that exact discussion and explicitly resolve it. Multica validates the current MR head and records every action; integration-authored notes do not trigger another run.
- **GitLab rebase state** — `need_rebase` is posted to linked issues once per MR head, separately from conflicts, unresolved discussions, CI, or requested changes.
